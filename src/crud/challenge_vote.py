...
def get_one(db: Session, challenge_vote_id: int) -> models.ChallengeVote | None:
    return (
...
def delete_one(db: Session, challenge_vote_id: int) -> None:
    db.query(models.ChallengeVote).filter(
        models.ChallengeVote.id == challenge_vote_id
    ).delete()
    db.commit()
